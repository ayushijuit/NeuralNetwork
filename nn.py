...
dataset = pd.read_csv("C:/Users/acer/Desktop/dataset.csv",encoding="latin1")
dataset.head()
...
embeddings_index = {}
with open('C:/Users/acer/AppData/Local/Programs/Python/Python37/Scripts/keras-text-summarization/demo/data/glove.6B.100d.txt', encoding='utf-8') as f:
    for line in f:
        values = line.split(' ')
        word = values[0]
...
max_text_length = 2125 # This will cover up to 89.5% lengthes
max_summary_length = 166 # This will cover up to 99% lengthes
min_length = 303
unk_text_limit = 10 # text can contain up to 1 UNK word
unk_summary_limit = 0 # Summary should not contain any UNK word



int_text_summaries = list(zip(int_summaries , int_texts))
sorted_int_text_summaries = sorted(int_text_summaries, key=lambda item: len(item[1]))
...
del int_text_summaries, sorted_int_text_summaries
# Compare lengths to ensure they match
print(len(sorted_summaries))
print(len(sorted_texts))

dump(clean_summaries, open('clean_summaries.pkl', 'wb'))
dump(clean_texts, open('clean_texts.pkl', 'wb'))
...
def model_inputs():
    input_data = tf.placeholder(tf.int32, [None, None], name='input') #placeholder used as a handle for feeding a value,but not evaluated directly
...
sorted_summaries_samples = sorted_summaries[0:35]
sorted_texts_samples = sorted_texts[0:35]
...
with train_graph.as_default():   # as_default() returns a context manager that makes a graph the default graph
...
input_sentence = clean_texts[0]
text = text_to_seq(clean_texts[0])
...
##
##summary=' {}'.format(" ".join([int_to_vocab[i] for i in answer_logits if i != pad]))
##summary_sentences=sent_tokenize(summary)
##testdocclass1=docx.Document('C:/Users/acer/Desktop/final yr project/testdocclass2.docx')
##wholedoc=""
